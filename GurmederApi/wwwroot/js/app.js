...
function getFilteredSorted(){
  ...
  let items = base.filter(r=>{
    ...
    if(q){
      const hay = (r.name + ' ' + r.category + ' ' + r.tags.join(' ') + ' ' + r.ingredients.map(i=>i.name).join(' ')).toLowerCase();
...
function toggleFav(id){
  ...
  // Giriş yapılmışsa sunucudaki hesaba da yazılır — favoriler böylece
  // aynı hesapla farklı cihazlarda da senkron kalır. Yerel değişiklik
  // (yukarıda) her durumda hemen uygulanır; ağ isteği başarısız olursa
  // sessizce yutulur (localStorage zaten güncel, bir sonraki girişte
  // syncGuestFavoritesToServer farkı kapatır) — favori butonuna tıklamak
  // bir ağ hatası yüzünden asla görünür şekilde başarısız olmamalı.
  if(typeof isLoggedIn === 'function' && isLoggedIn()){
    const method = adding ? 'POST' : 'DELETE';
    fetch(`/api/favorites/${id}`, { method, credentials:'include' })
      .then(res => res.ok ? res.json() : null)
      .then(ids => { if(ids && typeof currentUser !== 'undefined' && currentUser) currentUser.favoriteRecipeIds = ids; })
      .catch(()=>{ /* çevrimdışı — yerel durum zaten güncel */ });
  }
}
...
function openRecipe(id, opts){
  ...
  document.getElementById('stepsList').innerHTML = r.steps.map((s,i)=>`
    <div class="step-row">
      ...
      <div class="step-content"><b>${s.title}</b><p>${s.text}</p></div>
    </div>`).join('');
...
function renderIngredients(r){
  ...
  document.getElementById('ingList').innerHTML = r.ingredients.map((ing,i)=>{
    const {amount:amt, unit, name} = ing;
...
async function init(){
  ...
  // Oturum kontrolü (ve varsa favori senkronizasyonu) tariflerden SONRA,
  // ilk render'dan ÖNCE çalışır — bu sırayla renderPopular() her zaman
  // dolu bir RECIPES dizisi üzerinde çalışır, yarış durumu oluşmaz.
  await checkSession();
  hideStateScreens();
  renderApp();
}
